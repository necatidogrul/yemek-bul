...
export class MobileStorageService {
  ...
  static async cacheSearchResult(
    ...
    try {
      ...
      const cacheDuration =
        this.CACHE_DURATION[source.toUpperCase() as keyof typeof this.CACHE_DURATION] ||
      ...
      if (cacheKeys.length > 100) {
        ...
        const sortedKeys = cacheKeys
          .sort((a, b) => cache[a].metadata.timestamp - cache[b].metadata.timestamp)
          .slice(0, 20);
   ...
   */
  static async getCachedSearchResult(ingredients: string[]): Promise<CachedSearchResult | null> {
    try {
      const ingredientHash = this.generateIngredientHash(ingredients);
      const cache = await this.getSearchCache();
      ...
      const halfLife = (cached.metadata.expiresAt - cached.metadata.timestamp) / 2;
  ...
  static async addSearchHistory(
    ...
    try {
      ...
      await AsyncStorage.setItem(this.KEYS.SEARCH_HISTORY, JSON.stringify(history));
   ...
   * User preferences güncelle
   */
  private static async updateUserPreferences(ingredients: string[]): Promise<void> {
    try {
      ...
      ingredients.forEach(ingredient => {
        ...
        const index = prefs.frequentIngredients.findIndex(ing => ing === normalizedIngredient);
      ...
      await AsyncStorage.setItem(this.KEYS.USER_PREFERENCES, JSON.stringify(prefs));
    } catch (error) {
   ...
   */
  static subscribeToNetworkChanges(callback: (isOnline: boolean) => void): () => void {
  ...
  static async clearAllData(): Promise<void> {
    try {
      await Promise.all(Object.values(this.KEYS).map(key => AsyncStorage.removeItem(key)));
  ...
  static async getStorageInfo(): Promise<{
    ...
    try {
      ...
      const yemekBulucuKeys = allKeys.filter(key => key.startsWith('@yemek_bulucu:'));
